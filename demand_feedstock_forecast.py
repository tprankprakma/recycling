...
Takes the outputs of ev_forecast.py and battery_chemistry.py and computes:
  ...
  1. Gross new EV sales each year (net additions + replacements for retired EVs)
...
-----------
  ...
  Mineral demand(y) = gross_sales(y)
                      × avg_pack_size_kWh(y)
                      × sum_over_chemistries[ chemistry_share(y,c)
                                              × mineral_intensity(c, m) ]
 
  Feedstock(y) = retirements(y)
                 × avg_pack_size_kWh(y - LIFETIME_YEARS)
                 × chemistry_mix(y - LIFETIME_YEARS)
 
Inputs
------
  ev_forecast_{scenario}_2050.csv  — wide format, State × year (from ev_forecast.py)
  battery_chemistry.csv            — year/High-Nickel/LFP (from battery_chemistry.py)
 
Outputs (per EV scenario)
--------------------------------------------------------------
  mineral_demand_{ev_scenario}.csv
  mineral_demand_{ev_scenario}.png
...
-------
  Mineral intensities (kg/kWh):
    ...
    LFP: derived from LiFePO4 stoichiometry (MW = 157.757 g/mol,
    theoretical capacity 170 mAh/g, nominal voltage 3.2 V, 90% utilization).
...
CHEMISTRY_FILE = "data/output/battery_chemistry.csv"
 
# Battery end-of-life assumption (years after registration)
LIFETIME_YEARS = 15
 
# ── Average pack size (kWh) by year ───────────────────────────────────────────
# Source: IEA Global EV Outlook 2024. US sales-weighted average.
# Known values: ~50 kWh in 2016, ~80 kWh in 2022, ~80 kWh in 2024.
# Pre-2016: held at 50 kWh (earliest known value).
# 2016-2022: linearly interpolated between known anchors.
# Post-2022: held at 80 kWh (most recent reliably cited value).
# No forward projection is attempted beyond the data.
...
def load_chemistry(path: str) -> pd.DataFrame:
    """
    Load chemistry CSV (year, High-Nickel, LFP).
...
def get_chem_row(chem_df, year):
    """Return the chemistry row for a given year, with nearest-year fallback."""
    if year in chem_df.index:
        return chem_df.loc[year]
    nearest = min(chem_df.index, key=lambda y: abs(y - year))
    return chem_df.loc[nearest]
 
 
def run(ev_scenario: str, cumulative: pd.Series, chem_df: pd.DataFrame) -> pd.DataFrame:
    """
    Compute mineral demand and feedstock availability for one EV scenario.
    ...
    for y in years:
        ...
        chem_row  = get_chem_row(chem_df, y)
        ...
        if eol_evs > 0 and eol_year in cumulative.index:
            ...
            eol_chem      = get_chem_row(chem_df, eol_year)
...
def plot_results(df: pd.DataFrame, ev_scenario: str, out_path: str):
    ...
    # Panel 3: Demand vs feedstock for Li, Ni, Co
    ax = axes[1, 0]
    ...
    fig.suptitle(
        ...
        f"EV scenario: {ev_scenario}  |  Battery lifetime: {LIFETIME_YEARS} yr",
        fontsize=12
    )
    note = ("Mineral intensities: Olivetti et al. (2017); LFP from LiFePO4 stoichiometry. "
...
def main():
    ...
    for ev_scenario, ev_path in EV_FORECAST_FILES.items():
        ...
        print(f"  Computing: EV={ev_scenario}")
        df = run(ev_scenario, cumulative, chem_df)
        all_dfs.append(df)
 
        stem = f"mineral_demand_{ev_scenario}"
        ...
        plot_results(df, ev_scenario, f"{stem}.png")
 
    # Print summary for mid scenario
    all_dfs_nonempty = [d for d in all_dfs if not d.empty]
    if all_dfs_nonempty:
        combined = pd.concat(all_dfs_nonempty, ignore_index=True)
        if "mid" in EV_FORECAST_FILES and os.path.exists(EV_FORECAST_FILES["mid"]):
            sub = combined[combined["ev_scenario"] == "mid"]
